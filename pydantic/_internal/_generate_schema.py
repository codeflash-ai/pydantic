...
class GenerateSchema:
    ...
    def _unpack_refs_defs(self, schema: CoreSchema) -> CoreSchema:
        ...
        if schema["type"] == "definitions":
            ref_dict = self.defs.definitions
            for s in schema["definitions"]:
                ref_dict[s["ref"]] = (
                    s  # Directly assigning without function call to get_ref
                )
    ...
    def _apply_single_annotation_json_schema(
        ...
        if isinstance(metadata, FieldInfo):
            schema = self._process_field_info(schema, metadata)
        return schema
    ...
    def _process_field_info(
        self, schema: core_schema.CoreSchema, metadata: FieldInfo
    ) -> core_schema.CoreSchema:
