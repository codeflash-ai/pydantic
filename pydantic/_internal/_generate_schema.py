...
class GenerateSchema:
        ...
        with self.model_type_stack.push(typed_dict_cls), self.defs.get_schema_or_ref(
            ...
            with self._config_wrapper_stack.push(
                ...
                metadata = build_metadata_dict(
                    js_functions=[
                        partial(
                            modify_model_json_schema, cls=typed_dict_cls, title=title
                        )
                    ],
    ...
    def _dataclass_schema(
        ...
        with self.model_type_stack.push(dataclass), self.defs.get_schema_or_ref(
            ...
            with ExitStack() as dataclass_bases_stack:
                ...
                metadata = build_metadata_dict(
                    js_functions=[
                        partial(modify_model_json_schema, cls=dataclass, title=title)
                    ]
                )
    ...
    def _apply_single_annotation(
        ...
        ref = schema.get("ref")
        if ref is not None or schema["type"] == "definition-ref":
            if ref is not None:
            ...
            schema = schema.copy()
            schema["ref" if ref is not None else "schema_ref"] = new_ref  # type: ignore

        maybe_updated_schema = _known_annotated_metadata.apply_known_metadata(
            metadata, schema if ref is None else schema.copy()
        )
    ...
    def _apply_single_annotation_json_schema(
        ...
        if isinstance(metadata, FieldInfo):
            ...
            if json_schema_update or json_schema_extra:
                core_metadata = CoreMetadataHandler(schema).metadata
                annotation_functions = core_metadata.setdefault(
                    "pydantic_js_annotation_functions", []
                )
                annotation_functions.append(
