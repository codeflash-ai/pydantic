...
# Use a direct dictionary key lookup and avoid reassignment to `schema_type`.
...
def collect_definitions(
    ...
    def _record_valid_refs(
        ...
        ref = get_ref(s)
        if ref:
            defs.setdefault(ref, s)  # Faster conditional insert
...
def define_expected_missing_refs(
    ...
    if not allowed_missing_refs:
        return None  # Early return for common case

    refs = set(collect_definitions(schema).keys())  # Use set for faster diff

    expected_missing_refs = (
        allowed_missing_refs - refs
