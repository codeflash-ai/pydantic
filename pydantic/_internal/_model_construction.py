    ...
    * check for items which look like fields but are not (e.g. have no annotation) and warn.
    ...
    for var_name, value in namespace.items():
        ...
        if isinstance(value, ModelPrivateAttr):
            should_del_items.append(
                (
                    var_name,
                    f"Private attributes must not use dunder names; use a single underscore prefix instead of {var_name!r}.",
                )
            )
        elif isinstance(value, FieldInfo) and not is_valid_field_name(var_name):
            suggested_name = var_name.lstrip("_") or "my_field"
            raise NameError(
                f"Fields must not use names with leading underscores; use {suggested_name!r} instead of {var_name!r}."
            )
            ...
            if var_name not in raw_annotations or not is_classvar(
                ...
                should_del_items.append((var_name, None))
            ...
            if var_name in base_class_fields:
                ...
                raise PydanticUserError(
                    ...
                    "type annotation; if `{var_name}` is not meant to be a field, you may be able to resolve this "
                    "error by annotating it as a `ClassVar` or updating `model_config['ignored_types']`.",
                    code="model-field-missing-annotation",
                )

    for var_name, error in should_del_items:
        if error:
