...
from typing import Dict, Tuple, Type, Any, Callable, Generic, NoReturn
...
from pydantic.fields import FieldInfo, ModelPrivateAttr, PrivateAttr
...
def inspect_namespace(
    ...
    * check for items which look like fields but are not (e.g. have no annotation) and warn.
    ...
    for var_name, value in list(namespace.items()):
        if var_name in {"model_config", "__pydantic_extra__"}:
            continue
        elif (
            (
            ...
            or isinstance(value, all_ignored_types)
            ...
            if var_name.startswith("__"):
                raise NameError(
                    "Private attributes must not use dunder names; use a single underscore prefix instead of {var_name!r}."
                )
            elif is_valid_field_name(var_name):
                raise NameError(
                    'Private attributes must not use valid field names; use sunder names, e.g. {"_" + var_name!r} instead of {var_name!r}.'
                )
            ...
            raise NameError(
                f"Fields must not use names with leading underscores; e.g., use {suggested_name!r} instead of {var_name!r}."
            )
        elif var_name.startswith("__") or var_name in base_class_vars:
            continue
    ...
    for ann_name, ann_type in raw_annotations.items():
        if (
            ...
            if is_annotated(ann_type):
                metadata = typing_extensions.get_args(ann_type)[1:]
                private_attr = next(
