...
def _field_name_for_signature(field_name: str, field_info: FieldInfo) -> str:
    ...
    return (
        _alias_if_valid(field_info.alias)
        or _alias_if_valid(field_info.validation_alias)
        or field_name
    )
...
def _generate_signature_parameters(
    ...
    for param in islice(present_params, 1, None):  # skip self arg
        field = fields.get(param.name)
        if field:
            # exclude params with init=False
            if not getattr(field, "init", True):
                continue
            param = param.replace(name=_field_name_for_signature(param.name, field))
